...
layui.define(["jquery", "layer"], function (f) {
    ...
    var m = {
        ...
        flexible: function (n) {
            ...
            if (p != undefined && p != null) {
                layui.sessionData(n, {key: o, value: p})
            } else {
                layui.sessionData(n, {key: o, remove: true})
            }
            ...
            var p = o.scrollLeft();
            if ("left" === q) {
                o.animate({"scrollLeft": p - 120}, 100)
            } else {
                if ("auto" === q) {
                    var n = 0;
                    ...
                    o.animate({"scrollLeft": n - 120}, 100)
                } else {
                    o.animate({"scrollLeft": p + 120}, 100)
                }
            }
            ...
            if (t) {
                layui.data(m.tableName, {key: "theme", value: t});
                if (c == t) {
                    t = undefined
                }
            } else {
                layui.data(m.tableName, {key: "theme", remove: true})
            }
            try {
                m.removeTheme(top);
    ...
    m.chooseLocation = function (s) {
        ...
        var u = function (D, C) {
            AMap.service(["AMap.PlaceSearch"], function () {
                var F = new AMap.PlaceSearch({type: "", pageSize: z, pageIndex: 1});
                var E = [C, D];
        ...
        var n = function () {
            var C = {resizeEnable: true, zoom: A};
            x && (C.center = x);
            ...
            h("#ew-map-select-input-search").off("input").on("input", function () {
                ...
                AMap.plugin("AMap.Autocomplete", function () {
                    var F = new AMap.Autocomplete({city: "全国"});
        ...
        p += "   </div>";
        m.open({
            id: "ew-map-select", type: 1, title: o, area: "750px", content: p, success: function (C, E) {
    ...
    m.cropImg = function (q) {
        ...
        layui.use(["Cropper", "upload"], function () {
            ...
            function z() {
                ...
                var E = {
                    elem: "#ew-crop-img-upload", auto: false, drag: false, choose: function (F) {
                ...
                var B = {aspectRatio: v, preview: "#ew-crop-img-preview"};
                C = new y(D[0], B);
                h(".ew-crop-tool").on("click", "[data-method]", function () {
                    var G = h(this).data(), H, F;
                    if (!C || !G.method) {
                        return
                    }
                    G = h.extend({}, G);
            ...
            A += "   </div>";
            m.open({
                title: n, area: "665px", type: 1, content: A, success: function (B, C) {
    ...
    h(document).on("mouseenter", "*[lay-tips]", function () {
        ...
        j.tips(n, this, {
            tips: [o || 1, p || "#303133"], time: -1, success: function (r, s) {
                if (q) {
                    q = q.split(",");
                    var u = q[0], t = q.length > 1 ? q[1] : undefined;
    ...
    h(document).on("click", "*[ew-href]", function () {
        ...
        if (top.layui && top.layui.index) {
            top.layui.index.openTab({title: o ? o : "", url: n})
        } else {
            location.href = n
        }
    });
